...
Each boundary is drawn as a **secant** of the plate — a straight chord running rim to rim, tangent to a circle around the gnomon foot, rather than a spoke converging on it. They therefore cross one another into a rosette, and the circle they all touch reads as a ring around the cleared foot. Adjacent chords cross exactly on the hour ray between them, so the wedge opening outward from each crossing is that hour's band.

The numerals are *engraved* rather than painted: cut into the plate as a height field the renderer reads as bump, so they fill with shadow under a low sun and flatten out near noon, as a chiselled dial does. Because the chords cross, a numeral pinned to one fixed radius would be struck through by whichever chord passed there, so each one is placed at the point on its hour ray that stands clearest of every chord, and scaled to the room it has. That is why the numerals do not sit on one tidy circle — it is the price of boundaries that cross. By default a dial reads *apparent solar time*, which is not your clock: depending on where you sit in your time zone and how far the equation of time has drifted, solar noon can be an hour or more from 12:00 on your watch, and daylight saving adds another hour on top. `dial_time` in `[style]` chooses what the dial reads:
