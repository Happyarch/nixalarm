...
void test_long_shadows_are_clipped_not_dropped() {
  ...
  // Every boundary must be a RAY FROM THE GNOMON FOOT out to the rim. An hour
  // line is where the plane through the style and the light cuts the plate,
  // and every such plane contains the style, so every hour line has to pass
  // through the point where the style meets the plate -- here, the plate
  // centre. A boundary that converged nowhere would not be an hour line.
  double furthest_start = 0.0, shortest_end = 1e9, worst_offset = 0.0;
  for (const SceneRod& t : scene.rods) {
    if (t.material != DialMaterial::Tick) continue;
    furthest_start = std::max(furthest_start, std::sqrt(t.a.x * t.a.x + t.a.y * t.a.y));
    shortest_end = std::min(shortest_end, std::sqrt(t.b.x * t.b.x + t.b.y * t.b.y));
    // Perpendicular distance from the dial centre to the boundary's own line:
    // |(b-a) x (-a)| / |b-a|. Zero exactly when the line points at the foot.
    double dx = t.b.x - t.a.x, dy = t.b.y - t.a.y;
    worst_offset = std::max(worst_offset, std::fabs(dx * (-t.a.y) - dy * (-t.a.x)) / std::sqrt(dx * dx + dy * dy));
  }
  expect_true(worst_offset < 1e-9, "every boundary line converges on the gnomon foot");
  expect_near(shortest_end, scene.plate_radius, 1e-9, "every boundary runs out to the edge of the disc");
  expect_true(furthest_start < 0.1 * scene.plate_radius, "and starts at the foot, not somewhere out on the plate");
}

// The shadow only ever falls on the side of the foot away from the light, so
// an hour line is a RAY, not a full line through the centre. Drawing the
// backward half would mark hours that can never be indicated there -- the
// reference implementation this geometry follows discards exactly that half.
void test_boundaries_are_rays_not_lines_through_the_foot() {
  for (bool moondial : {false, true}) {
    DialScene scene = build_test_scene(moondial);
    bool all_one_sided = true;
    for (const SceneRod& t : scene.rods) {
      if (t.material != DialMaterial::Tick) continue;
      // Both ends must lie on the same side of the foot: the inner end is a
      // short way along the very direction the outer end runs off in.
      if (t.a.x * t.b.x + t.a.y * t.b.y <= 0.0) all_one_sided = false;
    }
    expect_true(all_one_sided, moondial ? "moon dial boundaries stay on the shadow side of the foot"
                                        : "sun dial boundaries stay on the shadow side of the foot");
  }
}
...
void test_numerals_sit_between_the_boundary_lines() {
  for (bool moondial : {false, true}) {
    ...
    // A boundary runs from the foot outward, so its direction is simply the
    // angle of its outer end.
...
int main() {
  ...
  test_boundaries_are_rays_not_lines_through_the_foot();
